...
interface CategoryClientsTabProps {
  categoryId: string
...
export function CategoryClientsTab({
  categoryId,
  clientCategories,
  allClients,
  ...
  const availableClients = allClients.filter(
    (client) => !clientCategories.some((cc) => cc.clientId === client.id)
  )
          ...
          <tbody className="divide-y divide-white/10">
            {clientCategories.map((cc) => (
              <tr key={cc.id} className="hover:bg-white/5">
                <td className="px-6 py-4 text-white">{cc.client.name}</td>
                <td className="px-6 py-4">
                  <select
                    value={cc.status}
                    onChange={(e) => handleStatusChange(cc.clientId, e.target.value)}
